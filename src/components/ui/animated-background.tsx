...
export function AnimatedBackground({ className = '' }: AnimatedBackgroundProps) {
  ...
  useEffect(() => {
    ...
    const drawBlob = (blob: typeof blobs[0], t: number) => {
      const x = (blob.x + Math.sin(t * blob.speed + blob.direction.x) * 0.1) * canvas.width;
      const y = (blob.y + Math.cos(t * blob.speed + blob.direction.y) * 0.1) * canvas.height;
      ...
      gradient.addColorStop(0.7, `rgba(${blob.color.r}, ${blob.color.g}, ${blob.color.b}, ${blob.color.a * 0.5})`);
  ...
  return (
    <div className={`fixed inset-0 -z-10 ${className}`} style={{ zIndex: -1 }}>
      <canvas
        ...
        style={{ 
          background: 'linear-gradient(135deg, #fef7f0 0%, #f8f4ff 50%, #f0f9ff 100%)',
          zIndex: -1
        }}
      />
    </div>
