...
function getViralityScoreStyle(score: number) {
  if (score >= 50) {
    ...
    return {
      containerClass:
        'bg-gradient-to-r from-red-500 to-orange-500 shadow-lg shadow-red-500/50 border border-red-400/50 px-2 py-1',
    ...
    return {
      containerClass:
        'bg-gradient-to-r from-orange-400 to-yellow-500 shadow-md shadow-orange-500/30 border border-orange-300/50 px-2 py-1',
    ...
    return {
      containerClass:
        'bg-white/15 backdrop-blur-sm border border-white/20 px-2 py-1',
...
export default function ReelCard({ reel }: ReelCardProps) {
  ...
  return (
    <div
      className='relative overflow-hidden rounded-2xl border border-white/20 shadow-2xl shadow-pink-500/10 hover:shadow-3xl hover:shadow-pink-500/20 transition-all duration-300 group backdrop-blur-sm bg-white/5 hover:bg-white/10 hover:border-white/30 hover:-translate-y-1'
      dir='ltr'
    >
      <AspectRatio ratio={9 / 16}>
        ...
        >
          ...
          <div
            className={`absolute inset-0 bg-gradient-to-br from-gray-800 via-gray-700 to-gray-600 flex items-center justify-center ${reel.thumbnailUrl && !imageError ? 'hidden' : ''}`}
          >
          ...
          <div className='absolute bottom-3 start-3 end-3 z-10'>
            <div className='flex items-end rtl:items-start justify-between'>
              ...
              <div
                className={`flex items-center gap-1 rounded-full ${viralityStyle.containerClass}`}
              >
