...
import { Plus, Edit, Trash2, Eye, EyeOff, Search, Filter, Lightbulb } from 'lucide-react';
...
export function StoryIdeaManager() {
  ...
  const { data: storyIdeas = [], isLoading } = useQuery<StoryIdea[]>({
    queryKey: ['story-ideas-admin', searchQuery, categoryFilter, storyTypeFilter],
    queryFn: async () => {
      const params = new URLSearchParams();
      if (searchQuery) params.append('search', searchQuery);
      if (categoryFilter !== 'all') params.append('category', categoryFilter);
      if (storyTypeFilter !== 'all') params.append('storyType', storyTypeFilter);
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['story-ideas-admin', searchQuery, categoryFilter, storyTypeFilter] });
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Partial<StoryIdeaFormData> }) => {
    ...
    onError: (error: Error) => {
      showStatusMessage(`خطا در به‌روزرسانی ایده استوری: ${error.message}`, 4000);
    },
  });
  ...
  const toggleActiveMutation = useMutation({
    mutationFn: async ({ id, isActive }: { id: string; isActive: boolean }) => {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error?.message || 'Failed to toggle story idea status');
      }
      return response.json();
    },
    onSuccess: () => {
  ...
  const handleCreate = () => {
    if (!formData.title.trim() || !formData.description.trim() || !formData.category || !formData.storyType) {
  ...
  const handleUpdate = () => {
    if (!editingIdea || !formData.title.trim() || !formData.description.trim() || !formData.category || !formData.storyType) {
      showStatusMessage('تمام فیلدهای الزامی را پر کنید!', 3000);
      return;
    }
  ...
  const handleDelete = (id: string) => {
    if (window.confirm('آیا مطمئن هستید که می‌خواهید این ایده استوری را حذف کنید؟')) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">مدیریت ایده‌های استوری</h2>
          <p className="text-gray-600">مدیریت ایده‌های استوری و تنظیمات آن‌ها</p>
        </div>
          ...
          <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-medium text-gray-700">عنوان ایده</label>
                  <Input
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-sm font-medium text-gray-700">نوع استوری</label>
                  <Select value={formData.storyType} onValueChange={(value) => setFormData({ ...formData, storyType: value })}>
                    <SelectTrigger className="mt-1">
              ...
              <div>
                <label className="text-sm font-medium text-gray-700">قالب پیشنهادی</label>
                <Textarea
              ...
              <div>
                <label className="text-sm font-medium text-gray-700">راهنمای استفاده</label>
                <Textarea
