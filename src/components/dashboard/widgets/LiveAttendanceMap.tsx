...
export function LiveAttendanceMap() {
  const totalPresent = attendanceData.reduce(
    (sum, dept) => sum + dept.present,
    0
  );
  const totalEmployees = attendanceData.reduce(
      ...
      <Card className='backdrop-blur-xl bg-white/10 border border-white/20 shadow-2xl hover:shadow-3xl transition-all duration-300'>
        ...
        <CardContent>
          ...
          >
            ...
            <div className="mt-3 w-full bg-gray-200 rounded-full h-2 overflow-hidden overflow-hidden">
              <OptimizedMotion
                className='h-2 rounded-full bg-gradient-to-r from-[#ff0a54] to-purple-500'
                initial={{ scaleX: 0 }}
                animate={{ scaleX: overallPercentage / 100 }}
              ...
              >
                <div className='flex items-center justify-between'>
                  <div className='flex items-center gap-3'>
                    <div
                      className={`w-10 h-10 ${getStatusColor(dept.status)} rounded-full flex items-center justify-center group-hover:scale-110 transition-transform`}
                    >
                ...
                <div className='mt-3 flex items-center gap-3'>
                  <div className="flex-1 bg-gray-200 rounded-full h-2 overflow-hidden overflow-hidden">
                    <OptimizedMotion
