...
export function QuarterlyGoals() {
  ...
  const onTrackGoals = quarterlyGoals.filter(g => g.status === 'on_track' || g.status === 'excellent').length;
  const avgProgress = Math.round(quarterlyGoals.reduce((sum, g) => sum + g.progress, 0) / totalGoals);
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-start gap-3">
                    <div className={`w-10 h-10 ${goal.bgColor} rounded-full flex items-center justify-center group-hover:scale-110 transition-transform`}>
                  ...
                  <div className="text-right">
                    ...
                    <Badge 
                      variant="outline" 
                      className={`text-xs ${
                        goal.status === 'excellent' ? 'bg-green-100 text-green-800 border-green-200' :
                        goal.status === 'on_track' ? 'bg-[#ff0a54]/10 text-[#ff0a54] border-[#ff0a54]/20' :
                        goal.status === 'at_risk' ? 'bg-yellow-100 text-yellow-800 border-yellow-200' :
                ...
                <div className="space-y-3">
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <motion.div
                      className={`h-2 rounded-full ${
                        ...
                        goal.status === 'on_track' ? 'bg-[#ff0a54]' :
                  ...
                  <div className="space-y-2">
                    <h5 className="text-sm font-medium text-foreground">نتایج کلیدی:</h5>
