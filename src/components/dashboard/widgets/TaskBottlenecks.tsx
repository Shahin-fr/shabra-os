...
import { AlertTriangle, Clock, User, FileText, ArrowRight, CheckCircle } from 'lucide-react';
...
export function TaskBottlenecks() {
  ...
  const criticalBottlenecks = bottleneckData.filter(b => b.priority === 'critical').length;
  const blockedTasks = bottleneckData.filter(b => b.status === 'blocked').length;
        ...
        <CardContent>
          ...
          >
            <div className="text-center p-3 rounded-xl bg-white/5">
              <div className="text-2xl font-bold text-foreground">{totalBottlenecks}</div>
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-start gap-3">
                    <div className={`w-10 h-10 ${getStatusColor(bottleneck.status)} rounded-full flex items-center justify-center group-hover:scale-110 transition-transform`}>
                  ...
                  >
                    {bottleneck.priority === 'critical' ? 'بحرانی' :
                     bottleneck.priority === 'high' ? 'بالا' :
                     bottleneck.priority === 'medium' ? 'متوسط' : 'پایین'}
                  </Badge>
                </div>
