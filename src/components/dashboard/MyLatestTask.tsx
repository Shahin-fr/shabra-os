...
export function MyLatestTask() {
  ...
  return (
    <Card className="backdrop-blur-xl bg-white/10 border border-white/20 shadow-2xl h-full">
      ...
      />
      <CardContent>
        <div className={`p-3 rounded-lg border-l-4 ${getPriorityColor(mockLatestTask.priority)} bg-white/5`}>
          <p className="text-sm font-medium text-gray-900 leading-relaxed">
            {mockLatestTask.title}
          </p>
          <div className="flex items-center gap-2 mt-2">
            <div className={`w-2 h-2 rounded-full ${getPriorityColor(mockLatestTask.priority).replace('border-l-', 'bg-')}`}></div>
            <span className="text-xs text-gray-600">
              {mockLatestTask.priority === 'high' ? 'اولویت بالا' : 
               mockLatestTask.priority === 'medium' ? 'اولویت متوسط' : 'اولویت پایین'}
            </span>
          </div>
        </div>
      </CardContent>
